...
## df+1
	...
	New cancel:	
		...
		- df+1,4~3,2
			...
			increased damage during Garuda Force
			doesnt consume Heat anymore
			This change also applies to ff+4~3,2 | ws2,4~3,2
					
		- df+1,4~3,2,u* | df+1,4~3,2,d*
...
## 3,1
	New cancel:	
		...
		- 3,1,4
			Safe mid Extension, guaranteed if second hit is CH
					
		- 3,1,4,4
			Unsafe delayable high damage mid, wallsplats, guaranteed if third hit connects 
					
		- 3,1,d+2
			Unsafe mid Extension, floor breaks, guaranteed if second hit is CH
...
## d+2
	- Reduced the recovery frames by "5F"
...
# Tips & Tricks
	- utilize his b3+4 to create and maintain distance
 	- his b+1 now guarantees a f2,1 or f2,1,2 if you have Garuda! 
  	- all df+1 strings all beat different things. use df+1,2 for a safe followup that beats interrupting his other followups. df+1,4 to beat sidestepping his other options... feel free to experiment!
   	- 'uf+1 HOLD' is a great way to cash out some damage at the end of a juggle, but also a very strong tool for pressure during garuda force. be careful, as it is quite linear! 
	...
	- after df3,3 (or df4,3,3) is great for pressure but also on hit grants you a combo! Example combo: d2 > ws.4 > f3,2,4~3 > df1,4~3,2. Dont overuse as its easily steppable.
